...
if (! class_exists('CSF_Options')) {
  class CSF_Options extends CSF_Abstract
  {
    ...
    public function ajax_save()
    {
      ...
      if (! $result) {
        wp_send_json_error(array('error' => esc_html__('Error while saving the changes.', 'adking-core')));
      } else {
    ...
    public function set_options($ajax = false)
    {
      ...
      if (wp_verify_nonce($nonce, 'csf_options_nonce')) {
        ...
        if (! $ajax && ! empty($response['csf_import_data'])) {
          ...
          $importing    = true;
          $this->notice = esc_html__('Settings successfully imported.', 'adking-core');
        }
        ...
        if (! empty($transient['reset'])) {
          ...
          $this->notice = esc_html__('Default settings restored.', 'adking-core');
        ...
        if (empty($this->notice)) {
          $this->notice = esc_html__('Settings saved.', 'adking-core');
        }
    ...
    public function add_options_html()
    {
      ...
      echo ($this->args['show_form_warning']) ? '<div class="csf-form-result csf-form-warning">' . esc_html__('You have unsaved changes, save your changes!', 'adking-core') . '</div>' : '';

      echo ($has_nav && $this->args['show_all_options']) ? '<div class="csf-expand-all" title="' . esc_html__('show all settings', 'adking-core') . '"><i class="fas fa-outdent"></i></div>' : '';

      echo ($this->args['show_search']) ? '<div class="csf-search"><input type="text" name="csf-search" placeholder="' . esc_html__('Search...', 'adking-core') . '" autocomplete="off" /></div>' : '';

      echo '<div class="csf-buttons">';
      echo '<input type="submit" name="' . esc_attr($this->unique) . '[_nonce][save]" class="button button-primary csf-top-save csf-save' . esc_attr($ajax_class) . '" value="' . esc_html__('Save', 'adking-core') . '" data-save="' . esc_html__('Saving...', 'adking-core') . '">';
      echo ($this->args['show_reset_section']) ? '<input type="submit" name="csf_transient[reset_section]" class="button button-secondary csf-reset-section csf-confirm" value="' . esc_html__('Reset Section', 'adking-core') . '" data-confirm="' . esc_html__('Are you sure to reset this section options?', 'adking-core') . '">' : '';
      echo ($this->args['show_reset_all']) ? '<input type="submit" name="csf_transient[reset]" class="button csf-warning-primary csf-reset-all csf-confirm" value="' . (($this->args['show_reset_section']) ? esc_html__('Reset All', 'adking-core') : esc_html__('Reset', 'adking-core')) . '" data-confirm="' . esc_html__('Are you sure you want to reset all settings to default values?', 'adking-core') . '">' : '';
      echo '</div>';
      ...
      foreach ($this->pre_sections as $section) {
        ...
        if (! empty($section['fields'])) {
          ...
          echo '<div class="csf-no-option">' . esc_html__('No data available.', 'adking-core') . '</div>';
        }
      ...
      if (! empty($this->args['show_footer'])) {
        ...
        echo '<div class="csf-buttons">';
        echo '<input type="submit" name="csf_transient[save]" class="button button-primary csf-save' . esc_attr($ajax_class) . '" value="' . esc_html__('Save', 'adking-core') . '" data-save="' . esc_html__('Saving...', 'adking-core') . '">';
        echo ($this->args['show_reset_section']) ? '<input type="submit" name="csf_transient[reset_section]" class="button button-secondary csf-reset-section csf-confirm" value="' . esc_html__('Reset Section', 'adking-core') . '" data-confirm="' . esc_html__('Are you sure to reset this section options?', 'adking-core') . '">' : '';
        echo ($this->args['show_reset_all']) ? '<input type="submit" name="csf_transient[reset]" class="button csf-warning-primary csf-reset-all csf-confirm" value="' . (($this->args['show_reset_section']) ? esc_html__('Reset All', 'adking-core') : esc_html__('Reset', 'adking-core')) . '" data-confirm="' . esc_html__('Are you sure you want to reset all settings to default values?', 'adking-core') . '">' : '';
        echo '</div>';
