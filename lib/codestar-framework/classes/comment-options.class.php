  ...
  class CSF_Comment_Metabox extends CSF_Abstract
  {
    ...
    public function add_comment_meta_box_content($comment, $callback)
    {
      ...
      if (! empty($this->args['show_restore']) || ! empty($this->args['show_reset'])) {
        ...
        echo '<span class="button csf-button-reset">' . esc_html__('Reset', 'adking-core') . '</span>';
        echo '<span class="button csf-button-cancel">' . sprintf('<small>( %s )</small> %s', esc_html__('update post', 'adking-core'), esc_html__('Cancel', 'adking-core')) . '</span>';
        echo '</label>';
        echo '</div>';
      }
