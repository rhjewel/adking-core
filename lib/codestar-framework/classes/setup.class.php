...
if (! class_exists('CSF_Setup')) {
  class CSF_Setup
  {
    ...
    {
      load_textdomain('adking-core', self::$dir . '/languages/' . get_locale() . '.mo');
    }
    ...
    public static function add_admin_enqueue_scripts()
    {
      ...
      // Main style
      wp_enqueue_style('adking-core', self::include_plugin_url('assets/css/style' . $min . '.css'), array(), self::$version, 'all');
      ...
      wp_enqueue_script('adking-core', self::include_plugin_url('assets/js/main' . $min . '.js'), array('csf-plugins'), self::$version, true);

      // Main variables
      wp_localize_script('adking-core', 'csf_vars', array(
        'color_palette'     => apply_filters('csf_color_palette', array()),
        'i18n'              => array(
          'confirm'         => esc_html__('Are you sure?', 'adking-core'),
          'typing_text'     => esc_html__('Please enter %s or more characters', 'adking-core'),
          'searching_text'  => esc_html__('Searching...', 'adking-core'),
          'no_results_text' => esc_html__('No results found.', 'adking-core'),
        ),
      ));
    ...
    public static function field($field = array(), $value = '', $unique = '', $where = '', $parent = '')
    {
      ...
      if (! empty($field['_notice'])) {
        ...
        $field            = array();
        $field['content'] = esc_html__('Oops! Not allowed.', 'adking-core') . ' <strong>(' . $field_type . ')</strong>';
        $field['type']    = 'notice';
        $field['style']   = 'danger';
      }
      ...
      if (! empty($field_type)) {
        ...
        if (class_exists($classname)) {
          ...
          echo '<p>' . esc_html__('Field not found!', 'adking-core') . '</p>';
        }
      } else {
        echo '<p>' . esc_html__('Field not found!', 'adking-core') . '</p>';
      }
