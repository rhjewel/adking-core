...
class Egns_Blog_Tags_Custom_Widget extends WP_Widget
{
    ...
    function __construct()
    {
        parent::__construct(
            ...
            __('Egns Blog Tags (Custom)', 'adking-core'),
            array(
                'description' => __('Displays blog tags in custom HTML structure', 'adking-core'),
            )
        );
    }
    ...
    {
        $title = isset($instance['title']) ? apply_filters('widget_title', $instance['title']) : 'Discover By Tags';
        ...
        if (!empty($tags)) {
            ...
            echo '<p>' . esc_html__('No tags found.', 'adking-core') . '</p>';
        }
    ...
    {
        $title = isset($instance['title']) ? $instance['title'] : 'Discover By Tags';
        $number_of_tags = isset($instance['number_of_tags']) ? absint($instance['number_of_tags']) : 9;
?>
        <p>
